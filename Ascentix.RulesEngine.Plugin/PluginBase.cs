...
namespace Ascentix.RulesEngine.Plugin
{
    ...
    public interface ILocalPluginContext
    {
        ...
        // IPluginExecutionContext2 carries the origin-channel signal (IsPortalsClientCall: Portal vs Standard).
        // Null when the host does not provide it (e.g. some test fakes), so callers must tolerate null.
    ...
    public class LocalPluginContext : ILocalPluginContext
    {
        ...
        /// <summary>IPluginExecutionContext2 (origin signals); null if the host does not provide it.</summary>
        public IPluginExecutionContext2 PluginExecutionContext2 { get; private set; }
        ...
        public LocalPluginContext(IServiceProvider serviceProvider)
        {
            ...
            // Request the v2 context for origin signals. `as` keeps it null-safe
            // when the host (or a test fake) does not register this interface.
            try
            {
                PluginExecutionContext2 = serviceProvider.GetService(typeof(IPluginExecutionContext2)) as IPluginExecutionContext2;
            }
            catch
            {
                // Host or test fake does not support IPluginExecutionContext2, so treat as null.
                PluginExecutionContext2 = null;
            }
